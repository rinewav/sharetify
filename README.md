...
| 中央サーバー (`packages/hub`) | 引き合わせ + アカウント / グループ / プレイリスト + 同時リスニング |
| PWA (`packages/web`) | **仮のデータは残っていない。すべて実データで動く** |
| デスクトップ (`packages/desktop`) | 内蔵サーバー + 取得処理 + 直結の受け口 + 聴取記録 |
...
- **合言葉による直結。検索・再生ともに中央を通さずに動作**
  - 初回再生 1.9 秒、二度目以降 0.02 秒（手元に残るため）
  - 直結の上での転送は 4.8MB を 139ms
...
自分の PC の node にだけ接続する。**繋ぎ方は合言葉 6 文字の入力だけ。**

```
PC 側アプリ ──▶ 中央に名乗り出て合言葉を受け取る
スマートフォン ──▶ 合言葉を入力
中央 ──▶ 両者の接続情報だけを取り次ぐ
以降 ──▶ PC とスマートフォンが直接やり取りする（中央は通らない）
```

中央がするのは引き合わせまで。音声はこの経路を通らない。
一度繋いだ合言葉は端末に覚えさせ、次回は自動で繋ぎ直す。

開発中は Tailscale 経由でも動かせる（`docs/ios-testing.md`）。こちらは足場で、
配布時に利用者へ求めるものではない。
...
**到達できなかったときの中継役 (TURN)。** 住宅網での直結成功率は 9 割前後で、
携帯回線を挟むともう少し下がる。配る前に要る。暗号化されたまま素通しさせ、
中身には触れない形で置く。

その先: プレイリストの並べ替え、他サービスからの取り込み、音声の加工。

### 済んでいること

- 合言葉による直結、検索・再生・ジャケット
- アカウント / グループ / 共有プレイリスト (中央サーバー経由)
- 端末に残す仕組み。PC が落ちていても手元の曲は鳴る
- 同時リスニング (時刻合わせの実測で ずれ 0ms / 往復 1ms)
- 曲の繋ぎ、再生キュー、並びのかき混ぜ
- 聴取記録の送信 (last.fm)
